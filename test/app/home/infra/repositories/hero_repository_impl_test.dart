...
void main() {
  ...
  group('fetch hero comics usecase', () {
    test('should return a list of result comic', () async {
      when(datasource.fetchHeroComics(any, any, any)).thenAnswer((_) async => <ResultComicModel>[]);

      final result = await repository.fetchHeroComics(1, 1, 1);

      expect(result.fold(id, id), isA<List<ResultComicModel>>());
    });

    test('should return a bad request exception when datasource emits a bad request exception', () async {
      when(datasource.fetchHeroComics(any, any, any)).thenThrow(HeroBadRequestException(''));
    ...
    test('should return a conflict exception when datasource emits a conflict exception', () async {
      when(datasource.fetchHeroComics(any, any, any)).thenThrow(HeroConflictException(''));
